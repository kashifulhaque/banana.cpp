...
Usage:
    uv run ./scripts/export_smollm2_weights.py [--output-dir weights/smollm2]
"""
        ...
        while True:
            ...
            if name in state_dict:
                ...
                if list(original.shape) == shape:
                    ...
                    if torch.allclose(
                        loaded[:10], original_flat[:10], atol=1e-6
                    ) and torch.allclose(loaded[-10:], original_flat[-10:], atol=1e-6):
                        tensor_count += 1
                    else:
                        print(f"  WARNING: Data mismatch for {name}")
                else:
                    print(
                        f"  WARNING: Shape mismatch for {name}: {shape} vs {list(original.shape)}"
                    )
            else:
...
def main():
    parser = argparse.ArgumentParser(
        description="Export SmolLM2 weights to binary format"
    )
    parser.add_argument(
        "--model-name",
        type=str,
    ...
    parser.add_argument(
        "--verify", action="store_true", default=True, help="Verify exported weights"
    )
    parser.add_argument(
        "--print-info",
        action="store_true",
    ...
    if not args.weights_only:
        print(f"Exporting tokenizer to {args.output_dir}...")
        export_tokenizer(tokenizer, args.output_dir)

    if args.verify:
        success = verify_weights(model, weights_path)
        if success:
            print("All weights exported and verified successfully!")
        else:
    ...
    print("  SmolLM2Tokenizer tokenizer;")
